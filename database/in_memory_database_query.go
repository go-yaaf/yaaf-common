...
// supported functions: count : avg, sum, min, max
func (s *inMemoryDatabaseQuery) Histogram2D(field, function, dim, timeField string, interval time.Duration, keys ...string) (out map[Timestamp]map[int]float64, total float64, err error) {
	return nil, 0, fmt.Errorf("not yet implemented")
}
